...
## 1. 개요
본 연구는 기업 간 거래 네트워크에서 핵심 기업을 식별하고 계층구조를 분석하기 위한 새로운 방법론을 제시한다. 기존의 단순 거리 기반 계층 구분의 한계를 극복하기 위해, 네트워크 중심성, 기업 규모, 거래 패턴 등 다양한 지표를 복합적으로 활용하는 앙상블 접근법을 개발하였다.

## 2. 기함기업 선정 방법론

### 2.1 복합 순위 산출
기업의 네트워크 내 위치와 경제적 규모를 동시에 고려하기 위해 PageRank와 매출액 순위의 기하평균을 활용하였다. 기하평균은 극단적인 순위 차이를 조정하고, 두 지표 간의 균형을 달성하는데 효과적이다.
...
### 2.2 조건부 선정 프로세스
두 단계의 필터링 과정을 통해 균형 잡힌 기함기업 후보군을 선정한다:
...
이 두 그룹을 통합한 후, 기하평균 순위를 기준으로 최종 100개 기업을 선정한다.
